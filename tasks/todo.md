...
## Phase 11 — v1.7: Swift Package Manager (document + verify) + a real bug fix found along the way
- [x] Research: Flutter 3.44.0 (this toolchain's pinned version) already defaults to SPM for iOS/macOS, CocoaPods as automatic fallback. Confirmed via user's AskUserQuestion answer: scope is "document + verify," not a new flag, not removing CocoaPods.
- [x] Docs updated to explain SPM is the default and that `permission_handler`'s SPM path auto-detects from Info.plist alone: `bricks/chameleon_app/__brick__/docs/architecture.md` (new section), `docs/guardrails.md`, `tasks/lessons.md` #15, `.claude/CLAUDE.md`, root `README.md`.
- [x] `chameleon doctor`: added a warning-only `Xcode` check (`xcodebuild -version`) — real, unmocked confirmation it detects Xcode 26.6 on this machine.
- [x] **Real bug found while verifying "Info.plist alone is enough"**: it wasn't — `--permissions` has never actually written anything to `ios/Runner/Info.plist` or `android/.../AndroidManifest.xml`. `pre_gen.dart` computed the right `usage_descriptions` (with TODO placeholder copy — clearly intended to be auto-inserted, unlike `--biometrics`'s Info.plist entry, which is explicitly documented as a manual step) but nothing ever wrote them anywhere, because neither file is a brick-templated file (both are `very_good_create`'s own, only `ios/Podfile` lives under `__brick__/ios/`). Confirmed the same gap exists in the private reference repo this was forked from — inherited, not introduced. `tool/checks.sh`'s permission-drift check *would* have caught this, but it's shown only as informational output in `e2e/run_matrix.sh`/CI, never gating, so it silently fired on every `--permissions` run since v1 without anyone reading its output closely.
- [x] Fixed: `pre_gen.dart` now also derives `android_permissions` (exact `permission_handler`-required Android manifest constants, verified per-permission via pub.dev, including the multi-permission cases: location → FINE+COARSE, contacts → READ+WRITE). `post_gen.dart` now patches both `Info.plist` (inserts each `usage_descriptions` entry before the closing `</dict>`) and `AndroidManifest.xml` (inserts `<uses-permission>` elements before `<application`) for real, post-generation, since neither file is brick-templated.
- [x] Re-verified for real: generated `--permissions camera,notification,location`, confirmed the actual `NSCameraUsageDescription`/`NSLocationWhenInUseUsageDescription` keys and `android.permission.CAMERA`/`POST_NOTIFICATIONS`/`ACCESS_FINE_LOCATION`/`ACCESS_COARSE_LOCATION` are genuinely present in the generated files (not just assumed), `flutter analyze`/`flutter test`/`dart run custom_lint` all clean, and `tool/checks.sh`'s permission-drift check no longer fires (only the unconditional, by-design failures remain: leftover TODOs, deep-link registration).
- [x] Confirmed real SPM resolution for the same app: `ios/Flutter/ephemeral/Packages/FlutterGeneratedPluginSwiftPackage` exists with `permission_handler_apple` listed among its resolved `.packages`, alongside `ios/Podfile`/`ios/Pods/` still present as the automatic CocoaPods fallback.
- [x] Re-ran the full `e2e/run_matrix.sh` (all 4 variants) after the fix to confirm no collateral regression, since it touches a hook every generated app runs through.
- [ ] Bump `chameleon_cli` to v0.3.0, tag `chameleon_cli-v0.3.0`, and create an actual GitHub Release (not just a git tag) marked as the repo's latest — per the user's explicit request to "tag it as the latest release."
